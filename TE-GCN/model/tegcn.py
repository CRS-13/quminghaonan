...
# from .LSTM.gcn_devLSTM import GCN_developRNN_basic
from .Attention import ActionRecognitionModel
# from .SFFusion3d import SqueezeAndExciteFusionAdd2D
...
class unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1):
        ...
        self.atten = ActionRecognitionModel(in_channels, out_channels)
        conv_init(self.conv)
        bn_init(self.bn, 1)
...
class TA_GCN3D_unit(nn.Module):
    def __init__(self, in_channels, out_channels, T, A, stride=1, residual=True, adaptive=True, attention=True,inherent=1,head=4,norm='ln',dropout=0.0,with_cls_token=0,pe=1):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.attention = attention
        # self.SFF = SqueezeAndExciteFusionAdd2D(in_channels)
    ...
    def forward(self, x):
        ...
        z += self.residual(x) ##原来的
        # print("z", z.shape)
        # print("res(x)", self.residual(x).shape)
        # z = self.SFF(z, self.residual(x))
        y = self.relu(z)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        # self.atten = EfficientAttention(256)
        # self.atten = ActionRecognitionModel(64, 64)
        ...
        if 0:
            attention = True
            adaptive = True
            self.l1 = TCN_GCN_unit(3, 64, A, residual=True, adaptive=adaptive, attention=attention)
        ...
        # self.LSTM = GCN_developRNN_basic(3, 256, 256, A)
