...
class Feeder(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=0.95, split='train', repeat=1, random_choose=False, random_shift=False,
    ...
    #     self.sample_name = [self.split + '_' + str(i) for i in range(len(self.data))]
        if self.angle:
            self.data = self.data.reshape((N, V, 2, 17, 9)).transpose(0, 4, 1, 3, 2)
        else:
            self.data = self.data.reshape((N, V, 2, 17, 3)).transpose(0, 4, 1, 3, 2)
