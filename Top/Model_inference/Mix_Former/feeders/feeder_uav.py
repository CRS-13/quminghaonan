...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        if self.split == 'train':
            ...
            self.data = npz_data['x_train']
            self.label = npz_data['y_train']#np.where(npz_data['y_test'] > 0)[1]
    ...
    def __getitem__(self, index):
        ...
        M, C, V, T = self.data[1].shape
        data_numpy = self.data[index]
        # data_numpy = data_numpy.transpose(1, 3, 2, 0)
        ...
        data_numpy = tools.valid_crop_resize(data_numpy, valid_frame_num, self.p_interval, self.window_size) # 3 64 17 2
